# Decision Briefs — settled and open calls

*Companion to `BUILD_STATE.md`. Principle: no decision is due before the
stage that needs it; every open decision has a working default. Saying “go”
authorizes the first Situation Globe code. Movement architecture is already
settled; First Light and the deterministic position layer require no further
design decision.*
