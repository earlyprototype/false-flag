...
The five stages in one line each: **1 Globe Display** — the map page exists and shows your forces · **2 Spatial State and Kinematics** — units get real positions that advance each turn · **3 Theatre API and Multi-Client Streaming** — the map reads the digital-twin model and many screens can watch · **4 Demo Operations and Reliability** — the demo runs to a written sequence without improvisation · **5 Validated Movement Orders** — your decision text moves the forces. Then the Afterwards tier — scheduled only when you say so.
