...
**E1 · Pyodide/GitHub-Pages exclusion — `CUT`, must be stated somewhere.** The browser build (the surface the owner actually plays; "merging to main is deploying") has no API server — the globe does not exist there in MVP. The competition demo story must run the FastAPI stack and say so explicitly, or accept a worker-bridge port as future work.
