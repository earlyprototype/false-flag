...
- **Track B — Authoritative spatial layer** (new, sanctioned by the reframing): typed unit positions become **engine state, written at each turn boundary**. The LLM never touches a coordinate: it emits **movement orders** (unit, mission, gazetteer destination, speed band) in one added non-blocking call; a pure, RNG-free kinematics module advances positions once per adjudication; deterministic **geometric tripwires** turn crossings into narrative events; the globe renders the engine's stored positions under source labels (`adjudicated` — written by this turn's kinematics step · `simulated` — projected forward between turns · `estimated` — the player's fog-filtered view). Failure behaviour is bounded by construction: clean parses apply; partial parses apply valid lines and visibly skip bad lines; empty, truncated, or failed calls issue zero new orders, every unit continues its last validated standing order, and kinematics advances. Only the gazetteer lookup at load and kinematics arithmetic at turn-resolve write positions, so a call failure can neither freeze nor invent one.
...
**Degradation ladder (normative).** Clean parses apply with provenance `adjudicated`. Partial parses apply valid lines and visibly skip bad lines with a **player-visible transcript line** plus `record_miss` (telemetry alone leaves half-executed intent invisible). Empty, mock-substituted, truncated, or failed calls issue **zero new orders; every unit continues its last validated standing order, and kinematics advances**, provenance `simulated`, with a dashboard counter so a campaign can't run "spatially deaf" unnoticed. Rationale: the call cannot freeze or invent a position; kinematics remains the only movement writer.
...
- **The contradiction risk reverses direction.** v1: the narrative text was the only record of events, so the globe could only lag it. Now unit positions are engine state, and *the narrative must stay consistent with them* — the movement call and the spatial context block exist precisely to keep the narrator grounded; residual drift is bounded by the validated-order degradation ladder.
